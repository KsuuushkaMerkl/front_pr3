...
const PORT = 8080;
const DATA_FILE = path.join(__dirname, '../catalog-server/desserts.json');
...
const server = http.createServer(async (req, res) => {
    if (req.method === 'GET' && req.url === '/desserts') {
        try {
    ...
    } else if (req.method === 'POST' && req.url === '/desserts') {
        let body = '';
    ...
    } else if (req.method === 'PUT' && req.url.startsWith('/desserts/')) {
    ...
    } else if (req.method === 'DELETE' && req.url.startsWith('/s/')) {
        const id = parseInt(req.url.split('/')[2], 10);
        try {
